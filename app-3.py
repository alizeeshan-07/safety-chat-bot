...
if st.button("Get Response"):
    try:
        with st.spinner("Model is working on it..."):
            ...
            # Here we assume you have a URL or a route to the document
            # Replace 'path_to_document' with the actual path to your document
            file_name, page = source_string.rsplit('_', 1)
            doc_url = f"path_to_document/{file_name}#page={page}"

            # st.markdown to create a hyperlink that opens in a new tab
            st.markdown(f"File: `{file_name}`, Page: [`{page}`]({doc_url}){{:target=\"_blank\"}}", unsafe_allow_html=True)
